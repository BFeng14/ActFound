...
for x in models:
    chembl[x] = []
    with open(os.path.join(f"/home/fengbin/meta_delta_master/result_ood/chembl", x, f"sup_num_{shot_name}.json"), "r") as f:
        res = json.load(f)
    for k in res:
        mean = 0
...
plt.show()
if ylabel == "r2":
    plt.savefig(f'./figs/2.e.figure_activity_domain_{ylabel}.pdf')
elif ylabel == "RMSE":
    plt.savefig(f'./figs/2.f.figure_activity_domain_{ylabel}.pdf')
