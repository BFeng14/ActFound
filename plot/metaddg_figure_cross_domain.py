...
chembl = {}
for x in models:
    with open(os.path.join("/home/fengbin/meta_delta_master/result_cross/bdb2chembl", x, "sup_num_16.json"), "r") as f:
        res = json.load(f)
    bdb[x] = []
    for k in res:
        mean = 0
    ...
    chembl[x] = []
    with open(os.path.join("/home/fengbin/meta_delta_master/result_cross/chembl2bdb", x, "sup_num_16.json"), "r") as f:
        res = json.load(f)
    for k in res:
        mean = 0
        d = np.mean([float(data[metric_name]) for data in res[k]])
