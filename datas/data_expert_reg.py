...
class CHEMBLMetaDataset(Dataset):
    ...
    def load_dataset(self):
        ...
        if self.args.datasource == "ood":
            experiment_train = preprocess.read_chembl_cell_assay_OOD()
            data_num = len(experiment_train["assays"])
            sup_num = 32
            test_num = 151
            self.split_name_train_val_test = {
                "train": experiment_train["assays"][:sup_num],
                "valid": [],
                "test": experiment_train["assays"][-test_num:],
            }
            self.assay_ids = experiment_train["assays"]
            ligand_set = experiment_train["ligand_sets"]
        elif self.args.datasource == "kiba":
            experiment_train = preprocess.read_kiba()
            sup_num = 128
            test_assayids = experiment_train["test_assay"]
            self.split_name_train_val_test = {
                "train": [x for x in experiment_train["assays"] if x not in test_assayids][:sup_num],
                "valid": [],
                "test": test_assayids,
            }
            self.assay_ids = experiment_train["assays"]
            ligand_set = experiment_train["ligand_sets"]
        elif self.args.datasource == "covid":
            experiment_train = preprocess.read_chembl_covid_assay()
            experiment_test = preprocess.read_covid()
            self.split_name_train_val_test = {
                "train": experiment_train["assays"],
                "valid": [],
                "test": experiment_test["assays"],
            }
            self.assay_ids = experiment_train["assays"] + experiment_test["assays"]
            ligand_set = {**experiment_train["ligand_sets"], **experiment_test["ligand_sets"]}
        else:
            print("no such data domain")
            exit()
        ...
        if self.args.train:
            for train_idx in self.train_indices:
                len_assay = len(self.Xs[train_idx])
                self.batch2trainidx += [train_idx] * math.ceil(len_assay / 200)
    ...
    def get_set(self, dataset_name, idx):
        ...
        for si in si_list:
            ...
            if self.args.use_gnn:
                smile_list = self.smiles_all[si]
                graphs = Batch.from_data_list([self.construct_graph(smile) for smile in smile_list], follow_batch=['x'])
            else:
                graphs = 0.
            graphs_all.append(graphs)
            datas.append(self.get_pair_data(self.Xs[si], self.ys[si], is_test=dataset_name in ['test', 'val'],
                                            scaffold_split=self.test_scaffold_split.get(si, None)))
...
class EXPERTMetaLearningSystemDataLoader(object):
