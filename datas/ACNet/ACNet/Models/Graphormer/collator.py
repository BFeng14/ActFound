...
def collator(items, max_node, multi_hop_max_dist, spatial_pos_max):
    data = []
    for item in items:
        [x,
         adj,
         ...
         label,
         idx]= item
        if item is not None and x.size(0) <= max_node:
            edge_input = edge_input[:, :, :multi_hop_max_dist, :]
            item = [attn_bias,
                    attn_edge_type,
                    spatial_pos,
                    in_degree,
                    ...
                    idx]
            data.append(item)
    attn_biases, attn_edge_types, spatial_poses, in_degrees, out_degrees, xs, edge_inputs, ys, idxs = zip(
        *data)
